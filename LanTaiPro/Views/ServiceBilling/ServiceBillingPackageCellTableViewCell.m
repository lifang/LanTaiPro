//
//  ServiceBillingPackageCellTableViewCell.m
//  LanTaiPro
//
...
@implementation ServiceBillingPackageCellTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier packageCardModel:(PackageCardModel *)package
{
    ...
    if (self) {
        NSArray *arrayOfViews = [[NSBundle mainBundle] loadNibNamed:@"ServiceBillingPackageCellTableViewCell" owner:self options: nil];
        if(arrayOfViews.count < 1){return nil;}
        if(![[arrayOfViews objectAtIndex:0] isKindOfClass:[ServiceBillingPackageCellTableViewCell class]]){
            return nil;
        }
...
-(void)leftButtonPressed:(id)sender {
    ...
    else
    {
        ...
        else
        {
            ...
            NSDictionary *aDic = [NSDictionary dictionaryWithObjectsAndKeys:@"add",@"remark", nil];
            [[NSNotificationCenter defaultCenter]postNotificationName:@"packageCardSelected" object:packageProduct userInfo:aDic];
        }
    }
...
-(void)rightButtonPressed:(id)sender {
    ...
    if ((count-1)<0) {
        ...
        NSDictionary *aDic = [NSDictionary dictionaryWithObjectsAndKeys:@"reduce",@"remark", nil];
        [[NSNotificationCenter defaultCenter]postNotificationName:@"packageCardSelected" object:packageProduct userInfo:aDic];
    }
